...
function Add-DnsSuffixList
{
  ...
  {
    Write-ErrorLog ("Failed to get availability zone: {0}" -f $_.Exception.Message)
  }
  ...
  {
    Write-ErrorLog ("Failed to get global search list: {0}" -f $_.Exception.Message)
  }
  ...
  {
    Write-ErrorLog ("Failed to get DNS domain from registry: {0}" -f $_.Exception.Message)
  }
  ...
  {
    Write-ErrorLog ("Failed to get NV domain: {0}" -f $_.Exception.Message)
  }
  ...
  {
    Write-ErrorLog ("Failed to get DNS domain from NICs: {0}" -f $_.Exception.Message)
  }
  ...
  {
    Write-ErrorLog ("Failed to set DNS suffix search list: {0}" -f $_.Exception.Message)
  }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAITDSt9XTWoV34
# APMkOgz9TUSmGDA1fxu6nXiDnzoYaaCCDlUwggawMIIEmKADAgECAhAIrUCyYNKc
...
# CQQxIgQgj8dq/scx+jqbaCqjotarUXMxMHuns8TlGsafsKNuSpMwDQYJKoZIhvcN
# AQEBBQAEggGASRygFNyUu6HEdgO7X+9PUauiHgM0PmespEx2SFZ2rH0LcmxajOvp
# RtYAk9+XCnhCFA/3dj1Ai1LPkx3+zIOb6VLpunXP8xVbftv9bZuL7oqbMElD2zbx
# Ohmmx5OjxYavk3v+kQD07ufDEdypM9FAb7AXRkYNM5cpEu94YsrZuByffG8L2vBB
# x64HZVuJVQdLif908okgEiicFVc8WZyDMTOVRqFcIEJsZRLhjHTtiZRBODay96nq
# Y5CdiOLNmoSql8Oc3GSQby2lgHBKibqDR8JEMO3DwyG2+/kfdxRY7BHn9PaEiEfP
# JlEsNbpKhtxcJWn+8r3QHVImm21IwD5TpS5rGRySbHV0om/P6relfsM2Yt6kyg9N
# dX6YzwwROiFAJX3zw1VdQcnkK75bZt5LkZNWaHBf0KgSlJX4fdLckGPfjozErtQD
# qN8a4rsrDO8TDciiKMOgAb+z/Rwd+jdeSiCHDjzaRuAX+fZteakiqe1tSAqo3muw
# mxKDwr7O6R5toYINfjCCDXoGCisGAQQBgjcDAwExgg1qMIINZgYJKoZIhvcNAQcC
# oIINVzCCDVMCAQMxDzANBglghkgBZQMEAgEFADB4BgsqhkiG9w0BCRABBKBpBGcw
# ZQIBAQYJYIZIAYb9bAcBMDEwDQYJYIZIAWUDBAIBBQAEIGb/8/41OSCRHIrFTAd8
# M2PU6JU1e8Nw1vUQWA+NGSkgAhEApPi4Mv0vQxY7924z8ys+3xgPMjAyMjAxMzEx
# ODQ4MThaoIIKNzCCBP4wggPmoAMCAQICEA1CSuC+Ooj/YEAhzhQA8N0wDQYJKoZI
...
# MQ8XDTIyMDEzMTE4NDgxOFowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQU4deCqOGR
# vu9ryhaRtaq0lKYkm/MwLwYJKoZIhvcNAQkEMSIEIHyiGTSU564HZ4BQRGliiaMa
# 7rCgxRJiLv2Jmp9FHCJJMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEILMQkAa8CtmD
# B5FXKeBEA0Fcg+MpK2FPJpZMjTVx7PWpMA0GCSqGSIb3DQEBAQUABIIBAIv0fXkH
# fwCoqLTa2ZdfLkakA3SRlrg8VOV+vvLXbojfdjeqnJEy+nSo0wvHMcj88cxXiF6A
# Mr0h/6w6siEKcus+HNsda+mMZl8vlYgx0X9nh6cs0VP4ahogAwuZpfG/dDoL5e1i
# 8WtqTDI8tMnWej5mN4esV7McIbljy5F5E5wkp/BO1y31H5LQL3lPZVT8Rymrpqw5
# 0ftbN4aBrceGXx0s0FcXJCoB6W2okWYUlBfwphLMlZ8Ck58R/FB66S+nKtV+Fw+K
# 0wKr3+CAbEy8MAZNXY/aB5jUweti4vI2AOEGkFbgmPBP74PkQWRe078EPDMdCS9p
# SPaHJilkR+RMPlg=
